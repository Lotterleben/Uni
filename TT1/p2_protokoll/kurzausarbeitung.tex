...
In Tabelle \ref{fig:ping_overview} finden sich die Daten der ping-Auswertung.

\begin{table}[ht]
 	\begin{tabular}{l|l|l}
		Eigenschaft & OLSR  & BATMAN \\
		\hline
		avg round trip time &TODO: ausfüllen!&\\
		max round trip time &&\\
		avg packet loss &&\\	
		max packet loss &&\\	
		avg Anzahl request timeouts &&
	\end{tabular}
	\caption{ping-Ergebnisse}
	\label{fig:ping_overview}
\end{table}
