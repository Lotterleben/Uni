...
//			 add potential predecessor to successor list and finger table and
	...
	public final void broadcast(Broadcast info) throws CommunicationException {
		...
		// copy fingertable because we sort it and stuff
		...
		logger.error("[BROADCAST]\n\t" 
				...
				+ "\n\tTxID:" + info.getTransaction()
				+ "\n\tFingertableSize:" + sortedFingertable.size());
//		
//		// The last element of the finger table has to receive a broadcast in which the range id
//        // is set to the id of this very node. Since the range id is always taken from the successor
//        // in the finger table, we add this node to the table and only iterate until the element next
//        // to the last.
		// sortedFingertable.add(this);
		
		Node currentNode;
		Node nextNode;
		int numFingerTableEntries = sortedFingertable.size();
		
		for (int i=0; i<numFingerTableEntries; i++) {
			currentNode = sortedFingertable.get(i);
			
			nextNode = null;
			if(i != numFingerTableEntries-1) {
				nextNode = sortedFingertable.get(i+1);
			}
			
			
			// Der Empfänger eines Broadcast-Pakets stellt dieses seiner Applikation zu und leitet es
			// an alle Knoten weiter, welche gemäß seiner Finger Table zwischen seiner eigenen ID und
			// dem RangeHash liegen. Dabei trägt er analog den jeweils darauffolgenden Finger Table Eintrag
			// in das RangeHash-Feld ein.
			//if(currentNode.getNodeID().isInInterval(getNodeID(), info.getRange())) {
			if(Util.isIDinRange(getNodeID(), info.getRange(), currentNode.getNodeID())) {
				
//				//ID updatedRange = nextNode.getNodeID();
//				if() {
//					updatedRange = info.getRange();
//				}
				
				ID updatedRange;
				if(nextNode != null && Util.isIDinRange(getNodeID(), info.getRange(), nextNode.getNodeID())) {
					updatedRange = nextNode.getNodeID();
				} else {
					updatedRange = info.getRange();
				}
